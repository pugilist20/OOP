...
public abstract class TFigure extends JPanel {
    ...
    public TFigure(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, Color color, int number) {
        this.tPoint.setX(x1);
        this.tPoint.setY(y1);
    ...
    public TFigure(int x1, int y1, int x2, int y2, Color color, int number) {
        this.tPoint.setX(x1);
        this.tPoint.setY(y1);
        this.tPoint1.setX(x2);
        this.tPoint1.setY(y2);
    ...
    public void MoveTo(int x, int y) {
        if(number==1){
            ...
            if(tPoint1.getX()<tPoint1.getY()) {
                if (tPoint1.getX()*1.5 + tPoint.getY() + y <= 605 && tPoint1.getX()/2+ tPoint.getX() + x <= 1250 && tPoint.getY() - tPoint1.getX() / 2 + y >= 5 && tPoint.getX() - tPoint1.getX() / 2 + x >= 5) {
                    tPoint.setX(tPoint.getX() + x);
                    tPoint.setY(tPoint.getY() + y);
                    this.Show(vision);
                    this.repaint();
                ...
                if (tPoint1.getY()/16  + tPoint.getY() + y <= 605 && tPoint1.getY()  + tPoint.getX() + x <= 1250 && tPoint.getY() - tPoint1.getY()  + y >= 5 && tPoint.getX() - tPoint1.getY() + x >= 5) {
                    tPoint.setX(tPoint.getX() + x);
                    tPoint.setY(tPoint.getY() + y);
                    this.Show(vision);
                    this.repaint();
