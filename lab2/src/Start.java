...
public class Start {
    ...
    ArrayList<Ring> listRing = new ArrayList<>();
    int selectedRing = -1;
    ...
    private boolean visionRing = false;
    ...
    private JPanel southPanel() {
        ...
        p.add(butRing).setBackground(Color.LIGHT_GRAY);
        ...
        butRing.addActionListener((e) -> {
            ...
            sP.repaint();
            if (!listRing.isEmpty()) {
                listRing.get(selectedRing - 1).setColor(Color.RED);
            }
            cP.revalidate();
            cP.repaint();
            numBut = 5;
        ...
        butCreate.addActionListener((e) -> {
            switch (numBut) {
                ...
                case 5:
                    ...
                    if (!listRing.isEmpty()) {
                        ring = new Ring(coords1, coords2, coords3, Color.RED);
                        listRing.add((Ring) ring);
                        if (listRing.size() >= 2) {
                            if (selectedRing == 0) {
                                listRing.get(selectedRing).setColor(Color.BLACK);
                            } else {
                                listRing.get(selectedRing - 1).setColor(Color.BLACK);
                            }
        ...
        butBack.addActionListener((e) -> {
            switch (numBut) {
                ...
                case 5:
                    ...
                    sP.add(butRing).setBackground(Color.LIGHT_GRAY);
                    break;
        ...
        butMove.addActionListener((e) -> {
            ...
            switch (numBut) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    if (selectedRing != 0 && selectedRing != -1) {
                        listRing.get(selectedRing - 1).MoveTo(coords1, coords2);
                        cP.repaint();
                    } else {
                        JOptionPane.showMessageDialog(title, "Кольцо не найдено");
                    }
                    coords1 = coords2 = 0;
                    break;
            }
        ...
        butDelete.addActionListener((e) -> {
            switch (numBut) {
                ...
                case 5:
                    if (listRing.size() > 1) {
                        int indexToRemove = (selectedRing == 1) ? 0 : selectedRing - 1;
                        cP.remove(listRing.get(indexToRemove));
                        listRing.remove(indexToRemove);
                        selectedRing = (selectedRing == 1) ? listRing.size() : selectedRing - 1;
                        listRing.get(selectedRing - 1).setColor(Color.RED);
                        cP.revalidate();
                        cP.repaint();
                    } else if (listRing.size() == 1) {
                        cP.remove(listRing.get(0));
                        listRing.remove(0);
                        selectedRing = -1;
                        cP.revalidate();
                        cP.repaint();
        ...
        butChangeRad.addActionListener((e) -> {
            if (numBut == 2) {
                ...
                if (selectedCircle != 0 && selectedCircle != -1) {
                    if(listCircle.get(selectedCircle - 1).getR()+coords1>=10){
            ...
            if (numBut == 5) {
                ...
                if (selectedRing != 0 && selectedRing != -1) {
                    ...
                    listRing.get(selectedRing - 1).chRad(coords1);
                    coords1 = 0;
                    cP.revalidate();
                    cP.repaint();
                } else {
            ...
            switch (numBut) {
                ...
                case 5:
                    if (selectedRing != 0 && selectedRing != -1) {
                        ...
                        listRing.get(selectedRing - 1).Show(visionRing);
                        cP.revalidate();
                        cP.repaint();
                    } else {
                        JOptionPane.showMessageDialog(title, "Кольцо не найдено");
        ...
        butPrevious.addActionListener((e) -> {
            switch (numBut) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    selectedRing = (selectedRing - 1 + listRing.size()) % listRing.size();

                    if (selectedRing == 0) {
                        selectedRing = listRing.size();
                        listRing.get(selectedRing - 1).setColor(Color.RED);
                        listRing.get(0).setColor(Color.BLACK);
                    } else {
                        listRing.get(selectedRing - 1).setColor(Color.RED);
                        listRing.get(selectedRing).setColor(Color.BLACK);
                    }
        ...
        butNext.addActionListener((e) -> {
            switch (numBut) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    selectedRing = (selectedRing) % listRing.size();
                    if (selectedRing == 0) {
                        listRing.get(0).setColor(Color.RED);
                        listRing.get(listRing.size() - 1).setColor(Color.BLACK);
                        selectedRing = 1;

                    } else {
                        listRing.get(selectedRing).setColor(Color.RED);
                        listRing.get(selectedRing - 1).setColor(Color.BLACK);
                        selectedRing += 1;
                    }
                    cP.repaint();
